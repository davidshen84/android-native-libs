pipeline {
  ...
  stages {
    stage('Download & unpack OpenSSL source') {
      when {
        expression { return !fileExists("${env['OPENSSL_ARCHIVE_NAME']}") }
      }
      steps {
    ...
    stage('Build OpenSSL') {
      steps {
        script {
          ...
          def abi_filter = "${params.ABI_FILTER ?: ''}".tokenize(' ')
          for (c in configs) {
            if (abi_filter.contains(c['abi'])) {
    ...
    stage('Download & unpack libcurl source') {
      when {
        expression { return !fileExists("${env['CURL_ARCHIVE_NAME']}") }
      }
      steps {
